...
def read_test_data(data_main_path, data_name, IMAGE_HEIGHT, IMAGE_WIDTH):
    ...
    mask = np.array(Image.open(image_path).convert('RGB'), dtype='f')
...
def depth2mesh(depth, mask, filename):
    ...
    for i in range(h):
        for j in range(w):
            f.write('v ' + str(float(2.0 * i / h)) + ' ' + str(float(2.0 * j / w)) + ' ' + str(
                float(depth[i, j, 0])) + '\n')
    ...
    for i in range(h - 1):
        for j in range(w - 1):
            ...
            dy_d = np.abs(dy_d)
            dx_r = np.abs(dx_r)
            if np.max(dy_u) < threshold and np.max(dx_l) < threshold and np.max(dy_d) < threshold and np.max(
                    dx_r) < threshold and mask[i, j]:
                f.write('f ' + str(int(j + i * w + 1)) + ' ' + str(int(j + i * w + 1 + 1)) + ' ' + str(
                    int((i + 1) * w + j + 1)) + '\n')
                f.write('f ' + str(int((i + 1) * w + j + 1 + 1)) + ' ' + str(int((i + 1) * w + j + 1)) + ' ' + str(
                    int(i * w + j + 1 + 1)) + '\n')
    f.close()
    return
